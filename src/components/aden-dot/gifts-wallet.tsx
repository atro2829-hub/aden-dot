    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="relative z-10">
          <p className="text-sm font-medium" style={{ color: `${NAVY}80` }}>{t('wallet.balance', lang)}</p>

          <div className="flex items-center gap-4 mt-3">
            <div className="flex items-center gap-2">
              <CoinIcon size={28} />
              <div>
                <p className="text-2xl font-bold" style={{ color: NAVY }}>{formatNumber(user?.coinsBalance || 0, lang)}</p>
                <p className="text-[10px]" style={{ color: `${NAVY}70` }}>{t('wallet.coins', lang)}</p>
              </div>
            </div>
            <div className="w-px h-10" style={{ background: `${NAVY}20` }} />
            <div className="flex items-center gap-2">
              <DiamondCurrencyIcon size={28} />
              <div>
                <p className="text-2xl font-bold" style={{ color: NAVY }}>{formatNumber(user?.diamondsBalance || 0, lang)}</p>
                <p className="text-[10px]" style={{ color: `${NAVY}70` }}>{t('wallet.diamonds', lang)}</p>
              </div>
            </div>
          </div>

          <div className="flex gap-2 mt-4">
            <Button className="flex-1 h-9 rounded-xl text-sm font-semibold" style={{ background: NAVY, color: GOLD }}>
              {t('wallet.deposit', lang)}
            </Button>
            <Button className="flex-1 h-9 rounded-xl text-sm font-semibold" variant="ghost" style={{ background: 'rgba(26,31,54,0.15)', color: NAVY }}>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { icon: <GiftIcon size={20} color={GOLD} />, label: t('wallet.giftShop', lang) },
      ...
      {/* Transactions */}
      <div>
        <h3 className="text-sm font-semibold text-white mb-3">{t('wallet.history', lang)}</h3>
        <div className="space-y-2">
          {transactions.map((tx, idx) => (
            ...
            >
              <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: tx.type === 'earn' ? '#22C55E15' : '#EF444415' }}>
                {tx.type === 'earn' ? (
              ...
              <div className="flex-1" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                ...
                <p className="text-[10px] text-gray-500">{new Date(tx.time).toLocaleDateString(lang === 'ar' ? 'ar-SA' : 'en-US')}</p>
              </div>
              <div className="flex items-center gap-1">
                {tx.currency === 'coins' ? <CoinIcon size={14} /> : <DiamondCurrencyIcon size={14} />}
                <span className={`text-sm font-semibold ${tx.type === 'earn' ? 'text-green-400' : 'text-red-400'}`}>
...
export function AchievementsPage() {
  ...
  const achievements = [
    ...
    { icon: <TrophyIcon size={24} color={GOLD} />, name: lang === 'ar' ? 'بطل' : 'Champion', desc: lang === 'ar' ? 'فز بـ 10 مسابقات' : 'Win 10 contests', progress: 100, completed: true },
    ...
    { icon: <ShieldIcon size={24} color={GOLD} />, name: lang === 'ar' ? 'حامي' : 'Protector', desc: lang === 'ar' ? 'أبلغ عن 5 محتويات' : 'Report 5 contents', progress: 40, completed: false },
      ...
      <div className="grid grid-cols-2 gap-3">
        {achievements.map((ach, idx) => (
          <motion.div
            ...
            style={{
              background: ach.completed ? `${GOLD}10` : 'rgba(255,255,255,0.03)',
              border: `1px solid ${ach.completed ? `${GOLD}30` : 'rgba(212,168,83,0.06)'}`,
            }}
          ...
          >
            ...
            {ach.completed ? (
              <div className="flex items-center justify-center gap-1 mt-2">
                <svg width={12} height={12} viewBox="0 0 24 24" fill={GOLD}><path d="M20 6L9 17l-5-5" /></svg>
                <span className="text-[10px] font-medium" style={{ color: GOLD }}>{t('achievements.completed', lang)}</span>
              </div>
            ) : (
              <div className="mt-2">
                ...
                <span className="text-[9px] text-gray-500">{ach.progress}%</span>
              </div>
            )}
          </motion.div>
        ))}
...
export function DailyRewardCard() {
  ...
  return (
    <motion.div
      ...
      style={{ background: `linear-gradient(135deg, ${NAVY}, ${GOLD}15)`, border: `1px solid ${GOLD}20` }}
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
    >
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-white">{t('achievements.dailyReward', lang)}</h3>
        <CoinIcon size={20} />
      </div>
      <div className="flex gap-1.5">
        {days.map((d) => (
          <div
            ...
            style={{
              background: d.claimed ? `${GOLD}20` : d.current ? `${GOLD}30` : 'rgba(255,255,255,0.03)',
              border: `1px solid ${d.claimed ? GOLD : d.current ? GOLD : 'rgba(255,255,255,0.05)'}`,
            }}
          >
            <p className="text-[9px] text-gray-400">{t('achievements.dayN', lang)} {d.day}</p>
            <div className="flex justify-center my-1">
              {d.claimed ? (
                <svg width={14} height={14} viewBox="0 0 24 24" fill={GOLD}><path d="M20 6L9 17l-5-5" /></svg>
              ) : (
                <CoinIcon size={14} />
              )}
            </div>
            <p className="text-[9px] font-medium" style={{ color: d.claimed || d.current ? GOLD : '#6B7280' }}>{d.coins}</p>
          </div>
        ))}
      </div>
      {!claimed && (
        <motion.button
          ...
          className="w-full mt-3 h-9 rounded-xl text-sm font-semibold"
