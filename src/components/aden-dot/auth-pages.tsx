...
export function LoginPage() {
  ...
  return (
    <motion.div
      className="min-h-screen flex flex-col justify-center px-6 py-8 relative overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 max-w-sm mx-auto w-full">
        ...
        <motion.div className="text-center mb-10" variants={fadeIn} initial="initial" animate="animate" transition={{ delay: 0.1 }}>
          <motion.div
            className="mx-auto w-24 h-24 mb-5 rounded-3xl flex items-center justify-center shadow-2xl"
            style={{ background: `linear-gradient(135deg, ${GOLD}, ${GOLD_LIGHT})`, boxShadow: `0 8px 32px ${GOLD}40` }}
        ...
        <motion.div className="mt-6" variants={fadeIn} initial="initial" animate="animate" transition={{ delay: 0.3 }}>
          ...
          <div className="flex gap-3">
            <motion.button
              className="flex-1 h-11 rounded-xl bg-white/5 border border-white/10 text-white text-sm flex items-center justify-center gap-2 hover:bg-white/10 transition-colors"
              whileTap={{ scale: 0.97 }}
            >
      ...
      <div className="relative z-10 max-w-sm mx-auto w-full">
        ...
        <div className="flex gap-1.5 mb-6">
          {[1, 2, 3, 4].map((s) => (
            <motion.div
              ...
              style={{ background: step >= s ? `linear-gradient(90deg, ${GOLD}, ${GOLD_LIGHT})` : 'rgba(255,255,255,0.08)' }}
        ...
        <motion.div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: `${GOLD}20`, color: GOLD }}>
              {step}
            </div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  autoFocus
                />
                <p className="text-[10px] text-gray-600">{lang === 'ar' ? 'أحرف إنجليزية وأرقام و _ فقط' : 'Letters, numbers, and _ only'}</p>
              </div>
            </motion.div>
          )}
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              ...
              {/* Preview */}
              <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(212,168,83,0.08)' }}>
                <Avatar className="w-12 h-12">
                  <AvatarFallback style={{ background: `${GOLD}20`, color: GOLD }} className="text-lg font-bold">
                    {nickname?.charAt(0) || '?'}
                  </AvatarFallback>
                </Avatar>
                <div>
        ...
        <AnimatePresence>
          {(error || localError) && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0, height: 'auto' }}
              exit={{ opacity: 0, y: -5, height: 0 }}
              className="bg-red-500/10 border border-red-500/20 text-red-400 text-sm p-3 rounded-xl mt-4"
            >
              {localError || error}
            </motion.div>
          )}
        ...
        <div className="mt-6">
          <Button
            ...
            style={{ background: `linear-gradient(135deg, ${GOLD}, ${GOLD_LIGHT})`, color: NAVY, boxShadow: `0 4px 16px ${GOLD}40` }}
          >
            {isLoading ? (
              <motion.div
                animate={{ rotate: 360 }}
...
export function VerifyEmailPage() {
  ...
  return (
    ...
    >
      {/* Background glow */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-96 h-96 rounded-full pointer-events-none" style={{ background: `radial-gradient(circle, ${GOLD}10, transparent)` }} />
      ...
      <div className="relative z-10 max-w-sm mx-auto w-full">
        ...
        <motion.div className="text-center mb-8" variants={fadeIn} initial="initial" animate="animate">
          ...
          >
            <svg width={36} height={36} viewBox="0 0 24 24" fill="none" stroke={GOLD} strokeWidth="1.8">
        ...
        <motion.div className="flex gap-2.5 justify-center mb-8" variants={fadeIn} initial="initial" animate="animate" transition={{ delay: 0.1 }}>
          {code.map((digit, i) => (
            <motion.input
              ...
              className="w-13 h-15 text-center text-xl font-bold rounded-xl border focus:outline-none focus:ring-2 transition-all"
              style={{
                width: '52px',
                height: '60px',
                ...
                color: 'white',
                boxShadow: digit ? `0 0 12px ${GOLD}30` : 'none',
                caretColor: GOLD,
              }}
        ...
        <AnimatePresence>
          {isVerifying && (
            <motion.div className="text-center mb-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              />
              <p className="text-sm text-gray-400 mt-2">{t('app.loading', lang)}</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <motion.div className="text-center" variants={fadeIn} initial="initial" animate="animate" transition={{ delay: 0.2 }}>
          {resendTimer > 0 ? (
            <p className="text-sm text-gray-400">
              {t('auth.resendIn', lang)} <span style={{ color: GOLD }}>{resendTimer}</span> {t('auth.seconds', lang)}
            </p>
          ) : (
            <button onClick={handleResend} className="text-sm font-semibold hover:underline transition-colors" style={{ color: GOLD }}>
            ...
            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="flex justify-center mb-8">
                ...
                >
                  ...
                  >
                    <Avatar className="w-full h-full border-4" style={{ borderColor: NAVY }}>
          ...
          {!isSent ? (
            ...
            <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
              <motion.div
                ...
                style={{ background: `${GOLD}15`, border: `2px solid ${GOLD}` }}
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                transition={{ type: 'spring', stiffness: 300 }}
              >
                <svg width={44} height={44} viewBox="0 0 24 24" fill="none" stroke={GOLD} strokeWidth="2.5">
